define(['moment'], function (moment) {
  (function (moment) {
    ...
    angular.module(moduleName, ['ngMaterial'])
      ...
      .directive('mdcDatetimePicker', ['$mdDialog', '$compile',
        function ($mdDialog, $compile) {
          ...
          return {
            ...
            link: function (scope, element, attrs, ngModel) {
              ...
              var sweetableTime = function (time) {
                var chechDate;
                if (time instanceof Date) {
                  time = moment(time).format("HH:mm");
                }

                chechDate = moment(time, "HH:mm");

                return chechDate;
              }

              ngModel.$validators.minDateValue = function (modelValue) {
                if (scope.minDate) {
                  var checkDate = sweetableTime(scope.minDate);
                  var currentDate = sweetableTime(modelValue);
              ...
              ngModel.$validators.maxDateValue = function (modelValue) {
                if (scope.maxDate) {
                  var checkDate = sweetableTime(scope.maxDate);
                  var currentDate = sweetableTime(modelValue);
